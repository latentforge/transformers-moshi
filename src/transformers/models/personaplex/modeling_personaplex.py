...
class PersonaPlexRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        PersonaPlexRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        # PersonaPlex scales by the weight in float32 (before casting back), unlike Llama which casts first.
...
class PersonaPlexAttention(nn.Module):
    ...
    def __init__(self, config: PersonaPlexConfig, layer_idx: int | None = None, use_flexible_linear=False):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
        self.num_key_value_groups = config.num_attention_heads // config.num_key_value_heads
        # PersonaPlex keeps the explicit `1 / sqrt(head_dim)` scaling to stay bit-exact with the reference impl.
        self.scaling = 1 / math.sqrt(self.head_dim)
        self.attention_dropout = config.attention_dropout
        self.is_causal = True
        # `PersonaPlexLinear` wraps either a plain `nn.Linear` (main decoder) or a per-codebook
        # `PersonaPlexFlexibleLinear` (depth decoder), selected by `use_flexible_linear`.
...
class PersonaPlexDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: PersonaPlexConfig, layer_idx: int, use_flexible_linear: bool):
        super().__init__()
        ...
        self.input_layernorm = PersonaPlexRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = PersonaPlexRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.use_flexible_linear = use_flexible_linear
...
class PersonaPlexPreTrainedModel(PreTrainedModel):
    ...
    _can_record_outputs = {
        "hidden_states": PersonaPlexDecoderLayer,
        "attentions": PersonaPlexAttention,
    }
...
@auto_docstring(
    custom_intro="""
    Transformer depth decoder consisting of *config.num_hidden_layers* layers, each one a [`PersonaPlexDecoderLayer`].
...
class PersonaPlexDepthDecoderModel(PersonaPlexPreTrainedModel):
    ...
    def __init__(self, config: PersonaPlexDepthConfig):
        ...
        self.layers = nn.ModuleList(
            [
                PersonaPlexDecoderLayer(config, layer_idx, use_flexible_linear=True)
...
@auto_docstring(
    custom_intro="""
    The PersonaPlex depth decoder with a per-codebook audio language modelling head on top.
    """
)
class PersonaPlexDepthDecoderForCausalLM(PersonaPlexPreTrainedModel, GenerationMixin):
    config: PersonaPlexDepthConfig
    ...
    def __init__(self, config: PersonaPlexDepthConfig):
        super().__init__(config)
        self.model = PersonaPlexDepthDecoderModel(config)
...
class PersonaPlexModel(PersonaPlexPreTrainedModel):
    def __init__(self, config: PersonaPlexConfig):
        ...
        # PersonaPlex reserves one extra id on top of the text vocabulary (used as the audio-only/BOS position)
        ...
        self.rotary_emb = PersonaPlexRotaryEmbedding(config=config)
        self.gradient_checkpointing = False
...
class PersonaPlexForCausalLM(PersonaPlexPreTrainedModel, GenerationMixin):
    # PersonaPlex's text embedding has one extra id (`vocab_size + 1`) compared to `lm_head`, so the two can never be tied.
    _tied_weights_keys = None
...
class PersonaPlexForConditionalGeneration(PersonaPlexPreTrainedModel, MoshiGenerationMixin):
    config: PersonaPlexConfig
    # Checkpoints published before the codec moved to `PersonaPlexProcessor` carry Mimi's weights under
    ...
    # `PersonaPlexFlexibleLinear` weights are 3D and do not map onto the standard colwise/rowwise strategies.
    _tp_plan = {"lm_head": "colwise_gather_output"}
    _pp_plan = {"lm_head": (["hidden_states"], ["logits"])}
    ...
    def __init__(self, config: PersonaPlexConfig):
        ...
        self.depth_decoder = PersonaPlexDepthDecoderForCausalLM._from_config(config.depth_decoder_config)

        # `num_codebooks` is the width of a single audio stream. The depth decoder predicts either just PersonaPlex's own
        # stream, or PersonaPlex's followed by the user's -- the released checkpoints do the former because the user-side
    ...
    def forward(
        ...
        **kwargs,
    ) -> PersonaPlexConditionalGenerationOutputWithPast:  # noqa: F821
        r"""
